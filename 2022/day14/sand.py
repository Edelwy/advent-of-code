...
def sand_falling(map, start, height, length):
    
    point = cp.copy(start)
    stopped = False

    while(not stopped):
        if point.y + 1 == height: return False
        elif map[point.y + 1, point.x] == False:
            point.y += 1
        elif point.x - 1 == -1: return False
        elif map[point.y + 1, point.x - 1] == False:
            point.x -= 1
            point.y += 1
        elif point.x + 1 == length: return False
        elif map[point.y + 1, point.x + 1] == False:
            point.x += 1
            point.y += 1
        else:
...
map = np.zeros(shape=(max_y + 1, max_x + 1), dtype=bool)
create_map(paths, map)

#reševanje:
units = 0
while(sand_falling(map, start, max_y + 1, max_x + 1)):
    units += 1
